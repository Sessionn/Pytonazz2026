...
class QueueView(discord.ui.View):
    ...
    def _build(self):
        ...
        # Row 0: ◀️  [pag1] [pag2] [pag3]  ▶️  — max 5 item
        prev = discord.ui.Button(
            emoji="◀️", style=discord.ButtonStyle.secondary,
            custom_id="qprev", disabled=(self.page == 0), row=0
        )
        ...
        if total > 2:
            ...
            for i in range(start, end):
                btn = discord.ui.Button(
                    label=str(i + 1),
                    style=(discord.ButtonStyle.primary if i == self.page
                           else discord.ButtonStyle.secondary),
                    custom_id="qpage_" + str(i), row=0,
                )
                btn.callback = self._make_page_cb(i)
                self.add_item(btn)

        nxt = discord.ui.Button(
            emoji="▶️", style=discord.ButtonStyle.secondary,
            custom_id="qnext", disabled=(self.page >= total - 1), row=0
        )
        nxt.callback = self._cb_next
        self.add_item(nxt)

        # Row 1: ✖️ Chiudi — sempre su riga separata
        close = discord.ui.Button(
            emoji="✖️", label="Chiudi", style=discord.ButtonStyle.danger,
            custom_id="qclose", row=1
        )
    ...
    async def _aggiorna(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            embed=queue_embed(self.player, self.page), view=self
        )
