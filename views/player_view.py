...
class PlayerView(discord.ui.View):
    ...
    def _build(self):
        ...
        self._add(discord.ui.Button(emoji="⏮️", style=discord.ButtonStyle.secondary, custom_id="prev", row=0), self._cb_prev)
        if p.is_paused:
            self._add(
                discord.ui.Button(emoji="▶️", label="Riprendi", style=discord.ButtonStyle.success, custom_id="toggle", row=0),
                self._cb_toggle,
            )
        else:
            self._add(
                discord.ui.Button(emoji="⏸️", label="Pausa", style=discord.ButtonStyle.primary, custom_id="toggle", row=0),
                self._cb_toggle,
            )
        self._add(discord.ui.Button(emoji="⏭️", style=discord.ButtonStyle.secondary, custom_id="skip", row=0), self._cb_skip)
        self._add(discord.ui.Button(emoji="⏹️", style=discord.ButtonStyle.danger, custom_id="stop", row=0), self._cb_stop)
        ...
        em, lb, st = loop_cfg[loop_mode]
        self._add(discord.ui.Button(emoji=em, label=lb, style=st, custom_id="loop", row=1), self._cb_loop)
        ...
        self._add(discord.ui.Button(emoji="🔀", label=shuffle_label, style=shuffle_style, custom_id="shuffle", row=1), self._cb_shuffle)

        self._add(
            discord.ui.Button(emoji="📝", label="Coda", style=discord.ButtonStyle.secondary, custom_id="queue_show", row=2),
            self._cb_queue,
        )
        if Config.DASHBOARD_PUBLIC_BASE_URL and p.guild:
            self.add_item(
                discord.ui.Button(
                    emoji="🎛️",
                    label="Console",
                    style=discord.ButtonStyle.link,
                    url=f"{Config.DASHBOARD_PUBLIC_BASE_URL}/dj-console?guild_id={p.guild.id}",
                    row=2,
                )
            )
        ...
        if vc:
            try:
                ...
                try:
                    ...
                    log.warning("_cb_stop: disconnect forzato fallito: %s", exc)
        await interaction.channel.send(
            embed=discord.Embed(
    ...
    async def _cb_queue(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=queue_embed(p, 0), view=view)
        try:
