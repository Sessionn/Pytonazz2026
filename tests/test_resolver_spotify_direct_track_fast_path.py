"""
tests/test_resolver_spotify_direct_track_fast_path.py

Esegui dalla root del progetto con:
    python tests/test_resolver_spotify_direct_track_fast_path.py
"""
...
def main() -> None:
    ...
    assert calls == ["ytsearch1:Jump All prodbykenny"], calls
    assert resolved is not None
    assert resolved.title == "Jump All", resolved
    assert resolved.source == "spotify", resolved
    assert resolved.spotify_url == "https://open.spotify.com/track/jump-all", resolved
    assert resolved.webpage_url == "https://soundcloud.com/prodbykenny/jump-all", resolved


main()
print("OK: spotify direct track uses fast ytsearch1 path when first match is strong")
