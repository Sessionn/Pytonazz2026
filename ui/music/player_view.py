...
class PlayerView(discord.ui.View):
    ...
    def _build(self):
        ...
        emoji, label, style = loop_cfg[loop_mode]
        self._add(discord.ui.Button(emoji=emoji, label=label, style=style, custom_id="loop", row=1), self._cb_loop)
    ...
    async def _cb_skip(self, interaction: discord.Interaction):
        ...
        await interaction.channel.send(
            embed=discord.Embed(
                description=f"Skip: **{skipped_title}** saltata da {interaction.user.mention}",
                color=0x5865F2,
            )
        )
    ...
    async def _cb_stop(self, interaction: discord.Interaction):
        ...
        await interaction.channel.send(
            embed=discord.Embed(
                description=f"Riproduzione interrotta da {interaction.user.mention}",
                color=0xFF0000,
            )
        )
    ...
    async def _cb_queue(self, interaction: discord.Interaction):
        ...
        if not p.current and not p.queue:
            await interaction.response.send_message(
                embed=discord.Embed(description="La coda e vuota.", color=0x5865F2),
                ephemeral=True,
            )
            return
