...
def build_command_embed(cmd, bot, cog_meta_fn, cmd_perm_fn, perm_badges, desc_prefix_markers) -> discord.Embed:
    ...
    icon, category = cog_meta_fn(bot, cog_key_str) if cog_key_str else ("*", "Altro")
    ...
    params = [
        param for param in (cmd.parameters if hasattr(cmd, "parameters") else []) if param.name != "interaction"
    ]
    if params:
        lines = []
        for param in params:
            optional = " *(opzionale)*" if not param.required else ""
            lines.append(f"`{param.name}`{optional} - {param.description or '-'}")
        embed.add_field(name="Parametri", value=trunc("\n".join(lines)), inline=False)

    usage = [f"/{full_name}"] + [f"<{param.name}>" if param.required else f"[{param.name}]" for param in params]
    embed.add_field(name="Utilizzo", value=trunc(f"`{' '.join(usage)}`"), inline=False)
...
def build_category_pages(key: str, cmds: list, include_dev: bool, bot, cog_meta_fn, page_size: int = 10) -> list[discord.Embed]:
    from core.help_utils import clean_desc as _clean
    from core.help_utils import cmd_full_name

    icon, label = cog_meta_fn(bot, key)
    color = 0x2F3136 if include_dev else 0x5865F2
    sorted_cmds = sorted(cmds, key=lambda cmd: cmd.name)
    pages = []
    for index in range(0, len(sorted_cmds), page_size):
        chunk = sorted_cmds[index : index + page_size]
...
def build_home_embed(all_pages: dict, include_dev: bool, bot, cog_meta_fn) -> discord.Embed:
    ...
    for index, (key, pages) in enumerate(items):
        icon, label = cog_meta_fn(bot, key)
        count = sum(len(page.fields) for page in pages)
        embed.add_field(name=f"{icon} {label}", value=f"`{count}` comandi", inline=True)
        remainder = (index + 1) % 3
        if index == len(items) - 1 and remainder != 0:
