...
from ui.help.embeds import build_home_embed
...
class _CategoryPagesView(discord.ui.View):
    ...
    def _stamp_footers(self):
        ...
        for index, page in enumerate(self.pages):
            page.set_footer(text=f"{self.category_label} | Pagina {index + 1}/{total} | < > per navigare")
    ...
    async def _go(self, inter: discord.Interaction, page: int):
        if inter.user.id != self.author_id:
            return await inter.response.send_message("Questo menu help non appartiene a te.", ephemeral=True)
        self.current = page
        self._update_buttons()
