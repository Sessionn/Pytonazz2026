...
class DevCog(commands.Cog, name="Dev"):
    ...
    async def cache_on(self, ctx: commands.Context) -> None:
        ...
        if missing:
            await ctx.reply(
                f"{WARN} Impossibile abilitare la cache.\n"
        ...
        await ctx.reply(f"{CHECK} Cache **abilitata**.", mention_author=False)
    ...
    async def cache_off(self, ctx: commands.Context) -> None:
        ...
        await ctx.reply(f"{PAUSE} Cache **disabilitata**. I dati restano nel DB.", mention_author=False)
    ...
    async def cache_status(self, ctx: commands.Context) -> None:
        ...
        icon     = CHECK if enabled else PAUSE
        env_icon = CHECK if env_ok else f"{WARN} variabili mancanti"
        lines = [
    ...
    async def cache_stats(self, ctx: commands.Context) -> None:
        ...
        if not Config.CACHE_ENABLED:
            await ctx.reply(f"{WARN} Cache non abilitata.", mention_author=False)
            return
        try:
            import core.cache_db as cdb
            s = cdb.stats()
